...
- [x] 键盘 ⌘, / ⌘1 / ⌘2；不加原生菜单（`.menu()` 是整体替换，会静默丢掉输入框里的 ⌘C/⌘V）
...
⌘, 开关设置面板、⌘1/⌘2 换线、Esc 关面板（Radix 自带，已确认没被吞）。只认纯 ⌘（`⌥⌘1` 之类是别人的组合键，别抢）；面板开着时不换线——换了当场也看不见，等关掉面板才发现自己已经不在原来那条线上了。
...
**当前状态**：**v1 已交付**——**M0 ~ M6 全部完成**（ADR-007~ADR-021），交付后又做了一轮**首页布局与交互链路重做**（ADR-023），§12 任务清单**无未勾选项**。三条压缩管线都已闭环并汇到同一个原子提交出口（校验 → no-gain 闸门 → 时间戳继承 → rename → fsync 父目录），上面接了调度器，再上面接了界面——**扫描 → 报告 → 开始 → 队列 → 重试这条主链路已经能整条走通**：
...
决议编号已用到 **D-176**，新决议从 D-177 起；风险编号已用到 **R22（已结案）**；基准测试编号已用到 **22**（基准 8 的规格在 §12.1，执行结果落为**基准 22**，见 ADR-021 §17）。
