...
`/private/tmp/zzvid/real/`（**不进 git**）：`cam720.mp4`（1280×720 实拍）、`ui720.mp4`、`motion1080.mp4`（1920×1080，602 帧 / 20.07 s，标定与回归主素材）、`screen.mov`（3456×2234 @58.7fps 录屏，同时覆盖缩放与降帧）。

`/private/tmp/zzaud/`（**不进 git**，均从真实音乐切片而来）：
...
- M3 只剩**双队列调度器**（D-07/D-08）一项，`core/orchestrator.rs` 尚未开工；`platform/power.rs` 目前只有 `PowerGuard`，热状态与低电量还没接。
...
**当前状态**：**M0、M1、M2 已完成，M3 只差双队列调度器**（ADR-007~ADR-014）。三条压缩管线都已闭环并汇到同一个原子提交出口（校验 → no-gain 闸门 → 时间戳继承 → rename → fsync 父目录）：
...
**下一步：M3 最后一项「双队列调度器」**（D-07/D-08，`core/orchestrator.rs` 尚未开工），含并发闸门与降级；`platform/power.rs` 目前只有 `PowerGuard`，热状态节流与低电量切硬编还没接（该项也出现在 M4 清单里）。图片侧只剩「基准 8 发布前验收」，规格见 §12.1，排在 M6 之后。

决议编号已用到 **D-75**，新决议从 D-76 起；风险编号已用到 **R22（已结案）**；基准测试编号已用到 **10**，**基准 8 仍预留给发布前验收**（规格见 §12.1，待执行）。
...
4. **新增任何一条处理分支时，同步检查「跳过判定 / 体积预估 / 落地闸门」三处**（D-74）。漏一个就会得到一条永远走不通、却没人报错的路。
