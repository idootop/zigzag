...
**~~D-227：release.yml 只挂 tag 触发，不加 `workflow_dispatch`。~~** → **已被 D-236 取代**
（2026-08-15）。原文的理由是：从分支手动跑的话 `github.ref_name` 是**分支名**，tauri-action
会照着建一个叫 `main` 的 release；真要试跑，push 一个 tag 再删掉即可——删 tag 和删 release
都是可逆的，而误建一条 release 不是。**这条观察本身没错，错在把它当成了「不能加手动触发」
的理由**——`ref_name` 读出分支名是可以绕开的（收敛到 `env.TAG`），而「构建挂了想重发」
比「试跑」更常发生。见下方 changelog 里 D-236 那条。
...
## 2026-08-18 · 质量门禁的抽样打分是错的：`-ss` 在两侧落到不同帧（ADR-034）

用户在另一台机器上拿 1.0.0 跑一块 1 TB 归档盘（约 50 万个文件），**623 个视频被
`low_quality` 跳过**。把 `jobs.profile_json` 里的 `video.vmaf_min` 从 80 改到 **50** 重跑，
这一轮处理了 12 条：**3 done / 8 还是 low_quality / 1 src_changed**——门槛砍掉 30 分几乎
没有作用。本轮就是去查这个「降到 50 还跳过」，结论是**产物一直是好的，坏的是量它的尺子**。

先记一条不影响数据安全的事实：原地模式下 `LowQuality` 走的是「删掉临时文件、原文件一个字节
不动」，所以这 623 条被错杀不丢数据，条目退回 `pending` 就能重跑。

### 1. 第一条铁证：同一个产物，两种量法差 77 分

拿本轮真被刷掉的 `VID20210703165448.mp4`（720p / 5.75 s，走的是「短片整段打分」那条路），
用**随包的** `/Applications/ZigZag.app/Contents/MacOS/ffmpeg` 原样复现应用的编码命令，
再用两种方式打分：

| 打分方式 | VMAF | libvmaf 配对的帧数 |
|---|---|---|
| 应用现行（两路 `setpts=PTS-STARTPTS`，即 D-71） | **22.70** | **173** |
| 两路 `settb=AVTB,setpts=N`（按帧号配对） | **99.84** | 172 |

源和产物**各 172 帧**，而应用那条链让 libvmaf 配出了 **173 对**——多出来的那一对就是错位的
证据：第 0 对 93.4 分，第 1~171 对 19~27 分，最后一对 100.0 分。

**像素级复核**（避免「是不是 VMAF 自己的问题」）：把产物第 30 帧和源的第 28/29/30/31/32 帧
逐个比 PSNR，得到 **12.75 / 14.44 / 32.18 / 14.16 / 12.28 dB**。第 30 帧对第 30 帧 32 dB，
差一帧就掉到 14 dB。所以：内容对得上、没有旋转/镜像问题，纯粹是**配对错了一帧**，
而手持晃动的素材上错一帧就等于两张不相干的图。

**注意 `setpts=N` 不能单独用**：两路 time_base 不同（1/90000 vs 1/11488），`N` 换算成绝对
时间不是同一个刻度，实测反而更差（9.29）。必须先 `settb=AVTB` 统一时基再 `setpts=N`。

### 2. 根因：`-ss` 在两侧落到不同的帧上，时间戳怎么改都救不回来

长视频走的是抽样路径（三个 2 s 窗口）。给两路输入同一个 `-ss`，然后 `showinfo` 看各自
交付的首帧偏移（`VID20210619202556.mp4`，720p / 46 s，帧间隔 33.4 ms）：

| 窗口 | 产物首帧偏移 | 源首帧偏移 | 相差 | 该窗 VMAF（现行 / 按帧号） |
|---|---|---|---|---|
| @6.9 s | 0.0286 s | 0.0016 s | **+0.81 帧** | 9.28 / 23.68 |
| @23.1 s | 0.0083 s | 0.0311 s | **−0.68 帧** | 34.90 / 13.44 |
| @39.3 s | 0.0203 s | 0.0154 s | +0.15 帧（同帧） | 18.48 / **96.07** |

**`-ss` 的精确 seek 是「丢掉 pts 小于目标的帧」，而源是手机的可变帧率、产物是编码器重排的
固定帧率，两条 pts 网格在目标时刻两侧的相对位置是随机的**——差不到半帧就对上，差过半帧就
整窗错开一帧。错位发生在「哪些帧被交付」这一步，等帧到了滤镜链里，
`setpts` 也好 `settb` 也好都已经无从补救。

这也解释了**为什么开发期没发现**：错位与否是逐文件、逐窗口随机的。本轮的对照组
（2304×1728 → 1440×1080）三个窗口恰好全部对齐，现行算法给出 92.28 分，完全正常。
ADR-014 的基准 10 就是量在这类「恰好对齐」的素材上的。

**D-71 不算错，只是不够**：它修的是「帧已经正确交付、但时间戳差 26 µs 导致配错」，
那个错真实存在（84.66 → 95.61）；它没能覆盖的是「一开始交付的就是不同的帧」。

### 3. 三条候选修法，实验否掉两条

| 候选 | 做法 | 结果 |
|---|---|---|
| A | **不 seek**，两路都 `select='between(n,…)'` 按帧号取三个窗口，再 `settb=AVTB,setpts=N` | **成立**，见下表 |
| B | 编码时 `-video_track_timescale` 对齐源时基，保留现行抽样 | **否决**：分数与现行**逐位相同**（20.89 / 42.20 / 64.96）。时基不是病根 |
| C | 编码时 `-fps_mode passthrough` 让产物继承源 pts | **不采纳**：解决不了 `fps=30` 那条路——参考端的 `fps` 滤镜在窗口内重采样，相位取决于窗口起点，与产物那条从 0 帧起算的网格天然不同相 |

A 的三个要点：`select` 必须放在 `vf` **之后**（这样 `n` 数的是降帧之后的帧，与产物同一个
计数）；`settb=AVTB` 必须在 `setpts=N` 之前；窗口末尾要留够 `WINDOW_SECS` 的帧。

### 4. 五个样本实测（前四个是本轮真被 50 分门禁刷掉的文件）

| 样本 | 规格 | 省 | 门禁现在给的分 | A 方案 | 独立交叉验证 |
|---|---|---|---|---|---|
| `VID20210703165448.mp4` | 720p 5.75 s（整段路径） | 10% | 22.70 | **99.84** | PSNR 36.89 dB / SSIM 0.9696 |
| `VID20210619202556.mp4` | 720p 46 s | 78% | 20.89 | **97.47** | 整段逐帧 VMAF **96.38**（最低帧 89.65）、PSNR 42.41 dB |
| `XEIV4602.MP4` | 888×1920 45.81→30 fps | 83% | 42.20 | **98.49** | PSNR 47.05 dB / SSIM 0.9986 |
| `video_20231223_095218.mp4` | 1080p 30.01→30 fps | 73% | 64.96 | **95.07** | PSNR 42.06 dB / SSIM 0.9842 |
| `scaled_src.mp4`（对照组） | 2304×1728→1440×1080 | 92% | 92.28 | **93.14** | PSNR 41.74 dB / SSIM 0.9849 |

两条结论：**真实画质 95~99.8，连原来 80 分的默认门槛都远远够过**（与 ADR-014
「默认档 96.13~99.04」一致）；**A 不会把本来对的样本搞坏**（对照组 92.28 → 93.14）。
`VID20210703165448` 只省 10%，修好门禁后它会改判成 `no_gain`（`min_gain_percent` = 20）——
这才是它该有的结论。

抽样的代表性也顺手验了：A 抽 180 帧给 97.47，整段 1383 帧给 96.38，差 1.1 分；
整段耗时 14.97 s，A 4.7 s。**抽样口径可以保留，只是不能靠 `-ss` 实现**。

### 5. A 的代价：从「与总长无关」变成「与总长成正比」

拿盘里最长的素材量吞吐（1080p HEVC / 2824 s / 4.7 GB，中段 60 s 切片）：

| 动作 | 60 s 内容的墙钟 | 折算 |
|---|---|---|
| 纯软解 | 8.67 s | 6.9× 实时 |
| `-hwaccel videotoolbox` 解码（回读到系统内存） | **5.26 s** | 11.4× 实时 |
| libx265 preset medium 编码 | 64.83 s | 0.93× 实时 |

A 要把两个文件都整段解一遍：软解 ≈ 编码耗时的 **27%**，加硬解 ≈ **17%**。
这是一笔真实的开销，换的是一个不会随机错杀的门禁——D-69 当初拒绝在软编路上加 `-hwaccel`
是因为并发下墙钟反而更慢，**但打分这条路上的 `-hwaccel` 是另一件事**（纯解码、无编码竞争），
要不要加得单独量一次并发下的表现。

### 6. 决议

> **D-237：质量门禁改成「按帧号取窗」，`-ss` 从打分路径上彻底拿掉。** 一次 ffmpeg 调用、
> 三个窗口一起打（现在是三次调用），`[0:v]select='between(n,…)+…',settb=AVTB,setpts=N`
> 对 `[1:v]<vf>,select='…',settb=AVTB,setpts=N`。窗口的帧号从**产物的 `nb_frames`** 换算，
> 而不是从源的帧率推——产物的帧数才是这套方法赖以成立的那个不变量。

> **D-238：`items` 加一列存 VMAF 分数。** 这个 bug 能活到 1.0 之后、能让用户对着 623 个
> 错杀文件反复调门槛，根本原因是**分数从来没落库**（`core/job.rs` 用 `Outcome::LowQuality { .. }`
> 把它丢掉了）。库里只有「跳过」没有「跳过时量到几分」，就没有任何办法在不重跑 ffmpeg 的
> 前提下判断门禁是不是在瞎跳。

> **D-239：`settings.json` 与 `jobs.profile_json` 是两份配置，改门禁要改后者。** 本轮为了
> 让运行中的任务生效，是直接 `json_set` 改的 `jobs.profile_json`（`store/repo.rs` 里
> `profile_json` 只有一处 INSERT、两处 SELECT，没有 UPDATE 路径，所以库里改完不会被启动流程
> 盖回去）。界面上改设置只影响**新扫描**。这条差异对用户是不可见的，值得在 UI 上给个说法。

### 7. 待办

- [ ] 按 D-237 改 `engines/vmaf.rs`（顺带把模块文档里基准 10 那张表标注成「量在恰好对齐的素材上」）
- [ ] 按 D-238 加列 + 落库 + 异常列表里显示分数
- [ ] 量一次打分路径加 `-hwaccel videotoolbox` 在双路并发下的墙钟，再决定加不加
- [ ] 门禁修好后，把 623 条退回 `pending` 重跑（`vmaf_min` 建议回到 80）
- [ ] 本轮的实测素材与对比图在两台机器的 `~/Desktop/zigzag-画质对比/`（含 `README-实测结论.txt`）
...
| 2026-08-15 | **release.yml 补手动指定 tag 发版**（`.github/workflows/release.yml`，参考 `doubless` 的同名工作流）。决议 **D-236，推翻 D-227**。起因是只挂 v-tag 触发时，构建中途挂掉、或者删了 release 想用同一个 tag 重发，都只能删 tag 再重推一遍。**D-227 当初拒绝 `workflow_dispatch` 的观察是对的、结论是错的**：从分支手动跑时 `github.ref_name` 确实是分支名（`main`），tauri-action 会照着建一个叫 `main` 的 release——但这是可以绕开的，不是「不能加手动触发」的理由。改法是**把两种触发方式的差别收敛到一行**：job 级 `env.TAG: ${{ inputs.tag \|\| github.ref_name }}`，脚本里 `$GITHUB_REF_NAME` 全换成 `$TAG`（版本断言、发版说明的 `{{VERSION}}` 替换各一处）。**`checkout` 必须显式 `ref:` 切到这个 tag**——手动触发时默认 checkout 的是触发分支的最新提交，不切的话打出来的包内容和 tag 指向的提交对不上，而**版本断言只比对三个文件里的版本号、这种偏差它一个字都测不出来**（断言会照样绿）。`concurrency.group` 从 `github.ref` 换成同一个表达式：tag 触发时它是 `refs/tags/v1.0.0`、手动触发时是 `v1.0.0`，不统一的话同一个 tag 能并发跑两趟。`tauri-action` 的 `tagName`/`releaseName` 直接写表达式没走 `env.TAG`（step 的 `with` 能否读 env context 没查到确凿依据，而跑到那一步已经烧掉几十分钟编译，不值得赌）。**两处没跟 doubless，都是单平台带来的**：矩阵里凡 bash 脚本都得显式 `shell: bash`（windows-latest 默认 pwsh，`set -euo pipefail` 会直接 ParserError），本项目只跑 macOS 用不着；action 版本沿用 v6 不动（口径见 D-233「一律取跑 node24 运行时的大版本」），不跟着 doubless 提到 v7。**验证只做到 YAML 层**：本机没装 actionlint，`yaml.safe_load` 过 + 三处表达式取值逐个核对，**语义级 lint 与真实运行都还没跑**——下次发版前先拿已存在的 `v1.0.0` 手动跑一趟确认能重发，再依赖它发新版 |
| 2026-08-18 | **ADR-034**：**质量门禁的抽样打分是错的——`-ss` 在两侧落到不同的帧上，把 95~99.8 分的产物判成 20~65 分**（纯实验轮次，无代码改动）。决议 **D-237~D-239**。起因是用户那块归档盘上 623 个视频被 `low_quality` 跳过，把 `vmaf_min` 从 80 降到 **50** 重跑仍然 8/11 继续跳。根因：`-ss` 的精确 seek 是「丢掉 pts 小于目标的帧」，而源是手机可变帧率、产物是编码器重排的固定帧率，两条 pts 网格在目标时刻两侧的相对位置**随机**——实测同一个文件三个窗口的首帧偏移差 **+0.81 / −0.68 / +0.15 帧**，对应 VMAF **9.28 / 34.90 / 96.07**；错位发生在「哪些帧被交付」这一步，**D-71 的 `setpts` 归零修不了**（它修的是帧已正确交付但时间戳差 26 µs 那个错，那个错真实存在）。像素级铁证：产物第 30 帧对源第 28/29/30/31/32 帧 PSNR = **12.75 / 14.44 / 32.18 / 14.16 / 12.28 dB**。五个样本改成「按帧号取窗」后实测 **99.84 / 97.47 / 98.49 / 95.07 / 93.14**，另有整段逐帧 VMAF 96.38、PSNR 36.9~47.1 dB、SSIM 0.970~0.999 三路独立交叉验证——**产物一直合格，坏的是尺子**。否决两条候选：`-video_track_timescale` 对齐时基（分数逐位相同，时基不是病根）、`-fps_mode passthrough`（救不了 `fps=30` 那条路的重采样相位）。代价是抽样从「与总长无关」变成整段解码两遍 ≈ 编码耗时的 27%（加硬解 17%）。**开发期没发现是因为错位逐文件随机，基准 10 恰好量在对齐的素材上**（本轮对照组同样恰好对齐，现行算法给 92.28 分完全正常）。另记 D-238：**分数从来没落库**，这才是这个 bug 能活到 1.0 之后的根本原因 |
...
决议编号已用到 **D-239**，新决议从 D-240 起；**ADR 已用到 ADR-034，新的从 ADR-035 起**；**提醒已用到 58**（44 起的几条只作为各自 ADR 里的引用块存在，没有回填到文末那份 1~43 的清单）；风险编号已用到 **R22（已结案）**；基准测试编号已用到 **24**（基准 8 的规格在 §12.1，执行结果落为**基准 22**，见 ADR-021 §17；**基准 23** 是感知指纹的重标定，见 ADR-031；**基准 24** 是发版流水线在 GitHub runner 上的耗时，见 ADR-033 §12）。
