...
**当前状态**：**M0 ~ M3 全部完成**（ADR-007~ADR-015）。三条压缩管线都已闭环并汇到同一个原子提交出口（校验 → no-gain 闸门 → 时间戳继承 → rename → fsync 父目录），上面接了调度器：
...
`cargo test` **302 项通过**（默认档 271 项 + `--ignored` 31 项真实素材，另 3 个基准），clippy 零告警。

**跑测试的三档**（ADR-016）：`cargo test`（7 s，不碰素材）／`cargo test -- --ignored --skip bench_`（41 s，真实编解码）／`cargo test --release -- --ignored bench_`（约 16 min，基准 11/12/13）。素材在仓库的 `fixtures/`（不进 git，`ZIGZAG_MEDIA` 可改指），**缺件即红灯**。

**下一步：M4「持久化与恢复」**——进度批量落库（500 ms / 200 条）、崩溃恢复（`running → pending` + 孤儿 `.zz-tmp` 清理）、源文件改动检测、暂停/继续/取消接到 UI。`core/orchestrator.rs` 的 `Event` 流就是落库与 UI 事件的接入点，目前只有回调、还没有消费者。`platform/power.rs` 仍只有 `PowerGuard`，热状态节流与低电量切硬编未接（**这是唯一预留的动态闸门收窄入口**，D-78 删掉的是随视频忙闲联动那一套，不是热节流）。图片侧只剩「基准 8 发布前验收」，规格见 §12.1，排在 M6 之后。

决议编号已用到 **D-82**，新决议从 D-83 起；风险编号已用到 **R22（已结案）**；基准测试编号已用到 **13**，**基准 8 仍预留给发布前验收**（规格见 §12.1，待执行）。
...
5. **跑并发/吞吐类基准一律 `--release`**（ADR-015 §3）。进程内 Rust 管线在 debug 下慢一个数量级，而 ffmpeg 子进程完全不受影响——两者一比较就会得到自洽但相反的结论。本轮差一点就把一条不该存在的降级规则写进代码。
6. **闸门宽度（视频 2 / 轻活 `ncpu-2`）是实测常数，不是可调参数**（D-80）。要改先重跑基准 11/12/13。
7. **默认 `cargo test` 不覆盖真实编解码**（D-82）。动过 `core/{video,image,audio}` / `engines/image.rs` / `platform/imageio.rs` 就得补跑 `--ignored --skip bench_`；新增依赖素材的用例记得挂 `#[ignore]` 并走 `crate::testutil::media()`。
