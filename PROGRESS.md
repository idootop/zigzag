...
> ⚠️ **欠账：六条一条都还没在真机 GUI 上验过。** 按项目规矩（提醒 15：界面这一层不能只靠 `tsc` 绿灯和单测绿灯），下面这六项验过之前不算完：
>
> 1. 镜像模式扫到一半强杀 → 重开 → 队列页**不该**出现「上次还剩 N 个」；正常跑一半强杀 → 重开 → 点「继续」**不必再选输出目录**（#1）
> 2. 压完一批点「完成」→ `zigzag.db` 当场缩回去（`ls -l` 前后对比）（#2）
...
**当前状态**：**v1 已交付**——**M0 ~ M6 全部完成**（ADR-007~ADR-021），交付后又做了一轮**首页布局与交互链路重做**（ADR-023）和一轮**试用反馈修复**（ADR-024，`tasks.md` 六条），§12 任务清单**无未勾选项**。三条压缩管线都已闭环并汇到同一个原子提交出口（校验 → no-gain 闸门 → 时间戳继承 → rename → fsync 父目录），上面接了调度器，再上面接了界面——**扫描 → 报告 → 开始 → 队列 → 重试这条主链路已经能整条走通**：
...
`cargo test` **474 项通过**（默认档）+ `--ignored` 47 项真实素材，另有 8 项基准单列；clippy 零告警，`tsc --noEmit` 与 `vite build` 通过，应用实机启动无报错。
...
**下一步：把 ADR-024 §7 的六项 GUI 验证跑掉**——`tasks.md` 那六条已经代码完成、门禁全绿，但**一条都还没在真机上验过**（提醒 15：界面这一层不能只靠 `tsc` 绿灯和单测绿灯；提醒 20：验之前先确认跑的是新代码）。验完之后 v1 仍然没有必做项。接手的 agent 要动手之前先看 §12 的「v2 候选（明确不进 v1）」，那是唯一有共识的待办池；另外 ADR-021 §17.11（D-163，按像素预算限流）记着一个**有意不做**的改进和它该被重新提起的信号。改动任何代码之前，先跑一遍下面「跑测试的三档」确认基线是绿的。

决议编号已用到 **D-187**，新决议从 D-188 起；风险编号已用到 **R22（已结案）**；基准测试编号已用到 **22**（基准 8 的规格在 §12.1，执行结果落为**基准 22**，见 ADR-021 §17）。
