...
impl Preset {
    ...
    pub fn description(self) -> &'static str {
        match self {
            Preset::Space => "体积最小，画质仍属高质量区间。适合空间吃紧的归档盘",
            Preset::Balanced => "推荐。画质接近视觉无损，典型可省约 90% 空间",
            Preset::Quality => "尽量保留原始画面，截图可达视觉无损。体积约为均衡档的 1.8 倍",
            // 「2~3 倍」是四组真实素材的实测区间（基准 9：1.84× / 2.02× / 2.51× / 3.43×，
            // 两侧 VMAF 基本持平）。已经压得很实的 720p 素材上硬编甚至会**变大**
            // （127%、123%），那种文件最后会被 no-gain 闸门整件丢掉。
            Preset::Fast => "视频走硬件编码，快 5~7 倍，但体积约为软编的 2~3 倍",
        }
    }
