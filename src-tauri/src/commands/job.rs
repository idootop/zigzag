...
///
/// 叫停会**当场掐掉在编的那几件**，连同它们的 ffmpeg 子进程（ADR-027）。这里
/// 原本写的是「让它们跑完，中途掐掉只会留下垃圾」——不成立：`Staged` 的 `Drop`
/// 会把 `.zz-*.tmp` 删掉，掐掉留不下垃圾；而让它们跑完的代价是界面已经停了之后
/// ffmpeg 还在 400~800% 的 CPU 上跑 16~44 秒（跟着在飞那件还剩多少走），并把
/// 用户刚喊停的产物提交进输出目录。
///
/// **正在跑时删是安全的**：任务收尾的那几条写全是 `UPDATE`（`apply_results`、
/// `set_job_status`、`release_running`），行没了就是影响 0 行，不会把任务写回来。
///
